...
if [ -d $DIR ]; then
  ...
  if [ -f $JAR_FILE ]; then
    java -jar -Dspring.profiles.active=discovery $JAR_FILE --manager.mode=run
    java -jar -Dspring.profiles.active=example-perf $JAR_FILE --manager.mode=run --manager.schedule=palisade-task
  else
