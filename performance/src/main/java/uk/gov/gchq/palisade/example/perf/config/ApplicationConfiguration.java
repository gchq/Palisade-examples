...
@EnableConfigurationProperties(value = {CreateConfiguration.class, RunConfiguration.class})
public class ApplicationConfiguration {
    @Bean
    @ConditionalOnBean(value = RunConfiguration.class)
    public Function<String, Stream<Employee>> configuredSimpleClient(final ExampleSimpleClient client, final RunConfiguration conf) {
        return resourceId -> {
            try {
    ...
    @Bean
    @ConditionalOnBean(value = RunConfiguration.class)
    public CommandLineRunner runAction(final RunConfiguration conf, final Set<PerfTrial> perfTrialSet) {
        Map<String, PerfTrial> testsToRun = perfTrialSet.stream()
                .collect(Collectors.toMap(PerfTrial::name, Function.identity()));
        return new ActionRunner(new RunAction(conf.getFilename(), conf.getDryRuns(), conf.getLiveRuns(), testsToRun, new HashSet<>(conf.getSkipTests())));
    }
    ...
    @Bean
    @ConditionalOnBean(value = CreateConfiguration.class)
    public CommandLineRunner createAction(final CreateConfiguration conf) {
        return new ActionRunner(new CreateAction(conf.getFilename(), conf.getSmall(), conf.getLarge()));
    }
}
