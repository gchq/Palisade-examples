...
if [ -f $JARFILE ]; then
  if [ -f $FORMATTER ]; then
    java -Dlogging.level.root=ERROR -Dlogging.level.uk.gov.gchq.palisade.example.runner.runner.RestExample=INFO -Dspring.profiles.active=k8s,rest -jar $JARFILE | $FORMATTER | tee $OUTPUT
  else
