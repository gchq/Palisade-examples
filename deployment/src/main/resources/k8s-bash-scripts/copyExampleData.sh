...
cp -vr /usr/share/example-jars/* /usr/share/deployment/classpath/example/
...
cp -vr /usr/share/example-data/resources/data/employee_file*.avro /data/local-data-store/
echo "Copied example-data to /data/local-data-store"

# Restart affected pods and wait until healthy
for service in {palisade,user,resource,policy,attribute-masking,filtered-resource,data,audit}-service; do
