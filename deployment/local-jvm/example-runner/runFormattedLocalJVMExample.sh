...
if [ -f $FILE ]; then
  if [ -f $FORMATTER ]; then
    java -Dlogging.level.root=ERROR -Dlogging.level.uk.gov.gchq.palisade.example.runner.runner.RestExample=INFO -Dspring.profiles.active=eureka,rest -jar $FILE | $FORMATTER
  else
